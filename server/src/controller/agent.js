...
router.get('/tree', async (ctx, next) => {
    ...
    let mongodb = global.mongodb
    //查出所有代理
    let agentArr = await mongodb.collection(Util.CollectionEnum.agent).find({ role: Util.RoleEnum.agent }, { projection: { id: 1, userName: 1, userNick: 1, status: 1, role: 1, createAt: 1, lastBalanceTime: 1, lastBalance: 1, _id: 0 } }).toArray()
